...
Template.AdventureSelectorActionTargetMenu.helpers({
  getActions: function () {
    ...
    if(node){
      return Collections.Actions.find({nodeId: node.staticId, projectVersionId: node.projectVersionId}, {sort: {title: 1}});
    }
  },
  getNavMenus: function () {
    if(this.node && this.node.navMenus){
      return Collections.Nodes.find({staticId: {$in: this.node.navMenus }, projectVersionId: this.node.projectVersionId}, {sort: {title: 1}});
    }
  }
});
