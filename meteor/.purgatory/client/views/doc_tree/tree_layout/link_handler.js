 ...
 * @constructor
 */
TreeLinkHandler = function (treeLayout, config) {
  var self = this;
 ...
 */
TreeLinkHandler.prototype.addLink = function(id, source, target){
  var found = false;
 ...
 */
TreeLinkHandler.prototype.prepLinks = function(){
 ...
 */
TreeLinkHandler.prototype.cleanLinks = function(){
  var i = this.linkList.length;
  while(i > 0){
    i--;
 ...
 * @param duration
 */
TreeLinkHandler.prototype.update = function (duration) {
  ...
  links = self.layer.selectAll(".link")
    .data(self.linkList, function(d){ return d.id; });
 ...
 */
TreeLinkHandler.prototype.createAndUpdateLinks = function (selection, duration) {
  var self = this;
  ...
  selection.enter()
    ...
    .attr("class", "link")
    .attr("opacity", 0)
    .attr("d", function(d, i){ return self.generateLinkPath(d, i); } );
