 ...
 */
export default class TreeInsetLayout {
  /**
   * TreeInsetLayout
  ...
  constructor(treeLayout, config) {
    ...
    if (!treeLayout) {
      console.error("TreeLayoutInsetLayout constructor failed: no TreeLayout passed");
      return;
    }
    ...
    // condense the config
    self.config = config || {};
    _.defaults(self.config, DocTreeConfig.insetView);
  ...
  update(duration) {
    ...
    nodes = self.nodeLayer.selectAll(".node-group")
        .data(nodeHandler.getNodes(), function (d) {
          return d._id;
        });
    ...
    links = self.linkLayer.selectAll(".link")
        .data(tree.linkHandler.linkList, function (d) {
          return d.id;
        });
