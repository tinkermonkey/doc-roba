...
Template.TestCaseStepWait.helpers({
  ...
  waitPartners() {
    if (this.data && this.data.waitId) {
      let stepIdMap      = [], // keep a handy lookup of the step Is so we know what to remove
  ...
  instance.updateRemovables = function (propagate) {
    ...
    if (refreshFailed) {
      instance.$(".removable-wait").draggable({
        ...
        stop(event, ui) {
          let keep     = ui.helper.data("dropped"),
...
Template.TestCaseStepWait.rendered = function () {
  ...
  instance.$(".droppable-wait").droppable({
    ...
    drop(event, ui) {
      ...
      let dragWaitId   = drag.attr("data-wait-id"),
      ...
      _.each(updates, function (update) {
        if (update.waitId) {
          let stepData    = TestCaseSteps.findOne(update.stepId).data || {};
