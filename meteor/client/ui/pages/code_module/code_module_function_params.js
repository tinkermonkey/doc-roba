...
Template.CodeModuleFunctionParams.events({
  "click .btn-add-param"(e, instance){
    let fn        = this,
        nextOrder = CodeModuleFunctionParams.find({ parentId: fn.staticId, projectVersionId: fn.projectVersionId })
            .count();
        ...
        update(event, ui) {
          ...
          instance.$(".sortable-table-row").each(function (i, el) {
            ...
            if (order != i) {
              ...
              CodeModuleFunctionParams.update($(el)
                  .attr("data-pk"), { $set: { order: i } }, function (error, response) {
                if (error) {
