 ...
 */
export default class TreeNodeHandler {
  /**
   * TreeNodeHandler
   * @param treeLayout
   * @param config
  ...
  mapNode(d) {
    ...
    _.each(d.childViews, function (node, i) {
      node.childIndex = i
    });
    _.each(d.childPages, function (node, i) {
      node.childIndex = i
    });
  }
  ...
  createRootNodes(selection) {
    ...
    selection
        ...
        .append("g")
        .attr("transform", function (d) {
          return "translate(-" + Math.round(d.bounds.width / 2) + ", -" + Math.round(d.bounds.height / 2) + ")"
        })
        .append("circle")
        .attr("class", "node node-root")
        .attr("cx", function (d) {
          return Math.round(d.icon.width / 2)
        })
        .attr("cy", function (d) {
          return Math.round(d.icon.width / 2)
        })
        .attr("r", function (d) {
          return d.icon.height / 2
        })
  ...
  createBacks(selection) {
    ...
    selection
        ...
        .append("g")
        .attr("transform", function (d) {
          return "translate(" + d.icon.left + ", " + d.icon.top + ")"
        })
        .append("rect")
