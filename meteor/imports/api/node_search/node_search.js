...
export const NodeSearch = {
  ...
  byUrl(url, title, projectVersionId) {
    ...
    Nodes.find({ projectVersionId: projectVersionId, type: { $in: [ NodeTypes.page, NodeTypes.view ] } })
    ...
    _.each(searchResults, function (result) {
      result.score.max = maxScore
    });
    
    // sort the results by score
    return _.sortBy(searchResults.filter(function (result) {
      return result.score.total
    }), function (result) {
      return maxScore - result.score.total
    });
  },
  ...
  byTerm(searchTerm, projectVersionId) {
    ...
    // sort the results by score
    return _.sortBy(searchResults, function (result) {
      return maxScore - result.score.total
    });
  },
   ...
   * @param context The adventure console instance
   */
  checkAdventureLocation(context) {
    var adventure = context.adventure.get(),
        state     = context.state.get();

    if (adventure && state) {
      var node = Nodes.findOne({ staticId: adventure.lastKnownNode });
      
      // check the local url
      if (node && node.url && state.url) {
        let result = NodeSearch.compareNode(state.url, state.title, node);
        
        if (result.url.match && result.params.match && result.title.match) {
          console.log("checkCurrentLocation, param match, location known");
          context.currentNodeId.set(node.staticId);
        } else {
          console.log("checkCurrentLocation, no param match, location unknown");
          context.currentNodeId.set();
        }
      } else {
        context.currentNodeId.set();
      }
  ...
  compareUrl(url, nodeUrl) {
    var result     = {
        ...
        pathPieces = Util.urlPath(url).split("/").filter(function (piece) {
          return piece.length
        }),
        i;
    
    if (nodeUrl) {
      var nodeUrlPieces = nodeUrl.split("/").filter(function (piece) {
        return piece.length
      });
