...
TreeNodeHandler = function (treeLayout, config) {
  ...
  // condense the config
  self.config = config || {};
  _.defaults(self.config, StatusTreeConfig.nodes);
 ...
 * @param nodeList The full flat list of nodes from the DB
 */
TreeNodeHandler.prototype.setNodesFromMap = function (nodeMap) {
  var self = this;

  // initialize the root note
  nodeMap.type = NodeTypes.root;

  // initialize the nodeList
  self.nextNodeId = 1;
  self.nodeList = self.nodeListFromMap([], nodeMap);
  //console.log("setNodesFromMap map:", nodeMap);
  //console.log("setNodesFromMap list:", self.nodeList);
};
 ...
 * @param nodeList The full flat list of nodes from the DB
 */
TreeNodeHandler.prototype.nodeListFromMap = function (list, node, parent) {
  var self = this;
  ...
  // Assign an ID
  n._id = self.nextNodeId++;
  n.staticId = n._id;
  n.parentId = parent && parent._id != undefined ? parent._id : null;

  // Set the type if needed
  if(n.type == undefined){
    n.type = NodeTypes.page;
  }
  ...
  _.each(node.children, function (child) {
    //list.concat(list, self.nodeListFromMap(list, child, n));
    self.nodeListFromMap(list, child, n);
  });
...
TreeNodeHandler.prototype.positionNodes = function () {
  ...
  _.each(self.getRootNodes(), function (rootNode){
    console.log("positionNodes:", rootNode);
...
TreeNodeHandler.prototype.calcNodeSize = function(d){
  var self = this,
      iconHeight  = self.config.height,
      iconWidth   = self.config.width;
  
  // get a size based on the type of node
  if(d.type == NodeTypes.root){
    iconHeight = 2 * self.config.rootRadius;
    iconWidth  = 2 * self.config.rootRadius;
  }
